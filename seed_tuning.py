...
def load_pairedregion_data(df, input_dim1, input_dim2, label_col, cfm_col):
    ...
    df2_layer1 = df[list(df.columns[4852:4852+input_dim2])].copy() # CHANGE BACK to df.columns[4851:4851+4841]
    df2_layer1['cfm'] = cfm2
...
def run_one_seed(df_input: pd.DataFrame, SEED: int):
    ...
    train_dataset = PairedDataset(df_train, input_dim, label_col, cfm_col, dataset)
    test_dataset = PairedDataset(df, input_dim, label_col, cfm_col, dataset)
...
if __name__ == "__main__":
    ...
    pd.DataFrame(seed_results).to_csv("/home/rachel/Desktop/mm-vae results/multiregion_adbxd_cr_final/seed_results1-50_multiregion.csv", index=False)