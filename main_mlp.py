...
# data_tensor = tor# if label_type == 'cfc': 
#     cfc_tensor = torch.tensor(dataset[['CFC.24mo.Average']].values, dtype=torch.float32)
# elif label_type == 'ymaze_distance':
#     cfc_tensor = torch.tensor(dataset[['Ymaze.Distance.10mo']].values, dtype=torch.float32)
# elif label_type == 'ymaze_speed':
#     cfc_tensor = torch.tensor(dataset[['Ymaze.MeanSpeed.10mo']].values, dtype=torch.float32)
# elif label_type == 'ymaze_alt':
#     cfc_tensor = torch.tensor(dataset[['SpontaneousAlternations.10mo']].values, dtype=torch.float32)ch.tensor(dataset[['LV1', 'LV2']].values, dtype=torch.float32)

# subset = TensorDataset(data_tensor, cfc_tensor)
...
# data = torch.tensor(data_df[['LV1', 'LV2']], dtype=torch.float32)
# cfc = torch.tensor(cfc_df['CFC.24mo.Average'], dtype=torch.float32).view(-1, 1)
