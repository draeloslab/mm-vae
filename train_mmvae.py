...
def train(epoch, model, optimizer, train_loader, num_samples, device, epochs, model_type, mode, train_osd=False, kl_annealing=False):
    model.train()
    total_loss = 0
    for i, data in enumerate(train_loader):
        ...
        if model_type == 'CGMVAE':
            if train_osd:
                ...
                loss, lpx_zs, kls, lpxz_ind, mse_loss, mse_cfm_loss, mse_lifespan_loss, osd_enc, osd_vals, bin_means, endpoints = model.moe_elbo_cgmvae_loss(x_data, y, cfm, lifespan, layer1_data, beta)
    ...
    return total_loss, lpx_zs, kls, lpxz_ind, mse_loss, mse_cfm_loss, mse_lifespan_loss, osd_enc, osd_vals, bin_means, endpoints

def test(epoch, model, optimizer, test_loader, num_samples, device, output_path, dataset_abbrev, physio_cols, mode):
    model.train()
    total_loss = 0
    with torch.no_grad():
        for i, test_data in enumerate(test_loader):
            model.reconstruct(test_data, output_path, epoch, dataset_abbrev, physio_cols, mode)
